...
cf)그래프 알고리즘은 bfs와 dfs 합집합 내에 있는 한 영역이라고 생각하기. 
=>그래프 알고리즘 문제이다->bfs/dfs알고리즘 사용하기(o). 그래프 문제가 아니더라도 bfs/dfs 알고리즘 사용할 문제들도 존재함
...
int solution(int n, vector<vector<int>> edge) {
    ...
    q.push(1);//1번 노드 push. 1번 노드를 0으로 표시하지 않고, 숫자 그대로 1로 사용하도록 코드 짬. 다른 노드들도 마찬가지임
